...
//    val output = Functions.stream(input).map(Integer.valueOf).collect(Collectors.toList)
...
//    val input = util.Arrays.asList("1", "2", "3", "4 ", "5", "6")
//    val testMethod = () => Functions.stream(input).map(Integer.valueOf).collect(Collectors.toList)
...
//        val testMethod = () => Functions.stream(input).forEach(asIntConsumer(ise))
...
//        val oomeTestMethod = () => Functions.stream(input).forEach(asIntConsumer(oome))
...
//        val seTestMethod = () => Functions.stream(input).forEach(asIntConsumer(se))
...
//    val input = util.Arrays.asList("1", "2", "3", "4", "5", "6")
//    val output = Functions.stream(input).map(Integer.valueOf).filter((i) => i.intValue % 2 eq 0).collect(Collectors.toList)
...
//    val input = util.Arrays.asList("1", "2", "3", "4", "5", "6")
//    val output = Functions.stream(input).map(Integer.valueOf).filter(asIntPredicate(null)).collect(Collectors.toList)
//    assertEvenNumbers(output)
//    Stream.of(dynamicTest("IllegalArgumentException", () => {
//      def foo() = {
//        val iae = new IllegalArgumentException("Invalid argument: " + 5)
//        val testMethod = () => Functions.stream(input).map(Integer.valueOf).filter(asIntPredicate(iae)).collect(Collectors.toList)
//        val thrown = assertThrows(classOf[IllegalArgumentException], testMethod)
...
//      def foo() = {
//        val oome = new OutOfMemoryError
//        val testMethod = () => Functions.stream(input).map(Integer.valueOf).filter(asIntPredicate(oome)).collect(Collectors.toList)
...
//      def foo() = {
//        val se = new SAXException
//        val testMethod = () => Functions.stream(input).map(Integer.valueOf).filter(asIntPredicate(se)).collect(Collectors.toList)
