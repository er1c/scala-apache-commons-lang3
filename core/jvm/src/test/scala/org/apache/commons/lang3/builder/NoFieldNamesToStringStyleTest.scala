...
/**
  ...
  */
class NoFieldNamesToStringStyleTest extends JUnitSuite {
  ...
  @Test def testObject(): Unit = {
    ...
    assertEquals(baseStr + "[<null>]", new ToStringBuilder(base).append("a", null.asInstanceOf[Any]).toString)
    ...
    assertEquals(baseStr + "[[]]", new ToStringBuilder(base).append("a", new util.ArrayList[AnyRef], true).toString)
    assertEquals(
      baseStr + "[<size=0>]",
